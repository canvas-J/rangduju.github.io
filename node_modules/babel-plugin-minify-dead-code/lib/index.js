    ...
    visitor: {
      ...
      IfStatement: {
        exit(path, _ref4) {
          ...
          var replacements = [];
          var renderFunction = path.findParent(function (p) {
            return p.isClassMethod() && t.isIdentifier(p.node.key) && p.node.key.name.startsWith("render");
          });
          var isRenderFunction = renderFunction && renderFunction.isClassMethod() && containJSXElement(renderFunction);
          ...
          if (evalResult.confident && evalResult.value) {
            path.replaceWithMultiple([].concat(replacements, _toConsumableArray(toStatements(consequent, isRenderFunction)), _toConsumableArray(extractVars(alternate))));
            return;
          }
          ...
          if (evalResult.confident && !evalResult.value) {
            if (alternate.node) {
              path.replaceWithMultiple([].concat(replacements, _toConsumableArray(toStatements(alternate, isRenderFunction)), _toConsumableArray(extractVars(consequent))));
              return;
            } else {
